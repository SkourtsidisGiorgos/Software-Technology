...
chai.use(chaiHttp);
const approx = require ('approximately-equal')
...
// GET MONTH
    ...
    it(chalk.cyan('Use Case 2 | 1.b:')+' GET /ActualTotalLoad/Austria/PT15M/month/2018-01', (done) => {
      ...
      .end((err, res) => {
          ...
          res.body[0].should.have.property('Year').that.is.a('number').equal(2018)
          res.body[0].should.have.property('Month').that.is.a('number').equal(1)
          res.body[0].should.have.property('ActualTotalLoadByDayValue').that.is.a('number')
          actual = res.body[0].ActualTotalLoadByDayValue
          done()
      })
  })
...
/*---------------------------------------------------------------------------------------------------*/
  ...
  it(chalk.cyan('Use Case 2 | 3.b:')+' GET /DayAheadTotalLoadForecast/Germany/PT15M/month/2018-01', (done) => {
    ...
    .end((err, res) => {
        ...
        forcasted = res.body[0].DayAheadTotalLoadForecastByDayValue
        done()
    })
})
...
/*it(chalk.cyan('Use Case 2 | 4.b:')+' GET /ActualvsForecast/Germany/PT15M/month/2018-01', (done) => {
    chai.request(server)
    .get('/energy/api/ActualvsForecast/Germany/PT15M/month/2018-01')
    .set('x-observatory-auth', user_token)
    .send()
    .end((err, res) => {
        res.should.exist
        ...
        res.body.should.be.an('array')
        res.body[0].should.have.property('Source').that.is.a('string').equal('entso-e')
        res.body[0].should.have.property('Dataset').that.is.a('string').equal('ActualVSForecastedTotalLoad')
        res.body[0].should.have.property('AreaName').that.is.a('string').equal('Germany')
        res.body[0].should.have.property('AreaTypeCode').that.is.a('string').equal('CTY')
        res.body[0].should.have.property('MapCode').that.is.a('string').equal('DE')
        res.body[0].should.have.property('ResolutionCode').that.is.a('string').equal('PT15M')
        ...
        res.body[0].should.have.property('Month').that.is.a('number').equal(1)
        res.body[0].should.have.property('Day').that.is.a('number').equal(1)
        res.body[0].should.have.property('ActualTotalLoadByDayValue').that.is.a('number').equal(forcasted)
        //res.body[0].should.have.property('DayAheadTotalLoadForecastByDayValue').that.is.a('number').equal(forcasted)
        done()
    })
})
